...
Consider a random variable $X$ with bounded support (meaning it strictly lives within a finite range, like $a \le \|X\| \le b$). Because it cannot shoot off to infinity, its probability distribution $P(X)$ has finite moments and can be entirely represented by them.
If we can prove that all the moments of $X$ are completely unaffected by another RV $Y$, meaning $\mathbb{E}[X^k | Y] = \mathbb{E}[X^k]$ for every integer $k$, then we have mathematically proven $P(X|Y) = P(X)$. That is the high-level idea behind the functional perspective for statistical independence.

Usually, we reach for Pearson's correlation or covariance to measure this. But covariance only captures the first-order, linear relationship. It is a one-way street: if we have mean independence, $\mathbb{E}[X|Y] = \mathbb{E}[X]$, it mathematically guarantees zero covariance, $\mathrm{Cov}(X,Y) = 0$. But zero covariance does not guarantee mean independence, except in a few special edge cases, like when $Y$ is binary. This is the idea behind many linear concept erasure methods. They try to remove a sensitive attribute $Y$ from an embedding $X$ by finding a transformation where $\mathrm{Cov}(f(X), Y) = 0$. In the best-case scenario, this achieves mean independence, $\mathbb{E}[X|Y] = \mathbb{E}[X]$. There is no guarantee on the independence of higher-order moments.

So, how do we augment the simple covariance so we can conclude true statistical independence? Instead of relying on polynomial moments like $X^k$, which only work perfectly on bounded domains, we upgrade to a much more powerful and generic scenario. If we can show that $\mathbb{E}[f(X)|Y] = \mathbb{E}[f(X)]$ for any bounded, well-behaved function $f$, we achieve independence. In the language of covariance: $X$ and $Y$ are independent if and only if $\mathrm{Cov}(f(X), g(Y)) = 0$ for all well-behaved functions $f$ and $g$.
...
\sup_{\{g_{\scriptscriptstyle\mathcal{I}}\}} \;\; \sup_{f} \quad
 \mathrm{Cov}[g_{x^i}(X^i), f(Z^i)]^2
+\tau_x\,\mathrm{Cov}[g_x(X), f(Z^i)]^2
+ \tau_y\,\mathrm{Cov}[g_y(Y),f(Z^i)]^2 \\
\text{s.t.} \quad \sup_{g_s} \; \mathrm{Cov}[g_s(S), f(Z^i)] = 0, \quad
