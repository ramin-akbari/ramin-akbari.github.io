...
$$\inf_{\theta} \quad \overbrace{\mathrm{HSIC}(Z_\theta,S)}^{\text{Performs Erasure}} - \overbrace{\left(\mathrm{HSIC}(Z_\theta,Y) + \mathrm{HSIC}(Z_\theta,X)\right)}^{\text{Preserves Information}}$$
...
The iterative approach shown in the curve above demonstrates the performance of Obliviator, which we will discuss in the next section.


## Introducing Obliviator